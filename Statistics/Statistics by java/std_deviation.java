...
//        int arr2[]={80,90,88,92,83,79,95,87,86,81};
       ...
       for(int i=0;i<arr.length;i++)
       {
           total_sum_f1+=(arr[i][0]-avrg_f1)*(arr[i][0]-avrg_f1);
           total_sum_f2+=(arr[i][1]-avrg_f2)*(arr[i][1]-avrg_f2);
       }
       double   mean_dev_f1;
               mean_dev_f1=Math.sqrt((double)total_sum_f1/arr.length);
       double mean_dev_f2;
               mean_dev_f2=Math.sqrt((double)total_sum_f2/arr.length);
               
       System.out.println("SMD first: "+mean_dev_f1);
       System.out.println("SMD Second: "+mean_dev_f2);
